...
LEVELLOADER.level1 = function(nextlvl,args) {
  var checkpt;
  if(args) {
  ...
  this.level1_base(checkpt);
  this.level1_dropper();

  GAMEMUSIC.currSong=0;
	//  GAMEMUSIC.playAudio();
};
LEVELLOADER.level1_base = function(checkpt)
{
...
LEVELLOADER.level1_dropper = function()
{
...
var prepjump = ACTOR_FACTORY.prepJumpActor;
				...
				GAMEMODEL.gameSession.gameWorld.addActor(TA,'act');
				        ...
				        GW.gamePlayer.updatePosition({x:0,y:50});
								...
								grid.setGridData(2,3,4,4,'tile','filler',{filled:false,color:0});
								grid.setGridData(1,2,5,5,'tile','riser',{filled:false});
